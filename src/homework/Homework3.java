...
//	static Scanner sc = new Scanner(System.in); 
	...
	public void distinguishTriangle() {
		 ...
		 * distinguishes the type of triangle and whether it's a right triangle, 
		 * by comparing the three integers.
		 * It's notable that an equilateral(正三角形) triangle is theoretically also 
		 * a special case of an isosceles(等腰三角形) one.
		...
		int a = 0, b = 0, c = 0;
		for (int i = 0; i < 3; i++) {
			// Use modulo to ensure each element in the array is compared with one another.
			a = triangle[i % 3];
			b = triangle[(i + 1) % 3];
			c = triangle[(i + 2) % 3];
		...
		// If none of the three triangle type conditions above is fulfilled, 
		// the type String would remain empty.
		if (type.isEmpty()) {
			type = "其他";
		}
	...
	public void guessNumber() {
		/*
		 * This method generates a random number between 0 and 100, as the answer, and
		 * keeps prompting user to enter a guessed number until the correct number is guessed.
	...
	public void lotteryPickNum() {
		 ...
		 * The remaining numbers are the result of filtering out the numbers having the unwanted digit.
		 ...
		 * To reduce space complexity, we shall declare an array with the length 
		 * as exactly the number of remaining numbers. 
		...
		for (int i = 0; i < 6; i++) {
			...
			// To ensure the newly generated random number is not duplicated with previous
			// ones.
			for (int j = 0; j < i; j++) {
